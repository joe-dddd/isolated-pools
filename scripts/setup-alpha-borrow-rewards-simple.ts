...
async function main() {
  ...
  const blocksPerYear = 42048000; // Back to original value

  // Deploy new RewardsDistributor
  console.log("Deploying RewardsDistributor...");
  const RewardsDistributor = await ethers.getContractFactory("RewardsDistributor");
  const rewardsDistributor = await RewardsDistributor.deploy(isTimeBased, blocksPerYear);
  console.log("Waiting for deployment confirmation...");
  await rewardsDistributor.deployTransaction.wait(2); // Wait for 2 confirmations
  console.log("RewardsDistributor deployed:", rewardsDistributor.address);

  // Initialize
  console.log("Initializing...");
  const initTx = await rewardsDistributor.initialize(comptrollerAddr, alphaTokenAddr, MAX_LOOPS_LIMIT, acmAddr);
  await initTx.wait();
  ...
  const alreadyAdded = rewardDistributors.some(
    (addr: string) => addr.toLowerCase() === rewardsDistributor.address.toLowerCase(),
  );
  if (!alreadyAdded) {
    console.log("Adding RewardsDistributor to Comptroller...");
    const tx = await comptroller.addRewardsDistributor(rewardsDistributor.address);
    await tx.wait();
    console.log("Added");
  }
  ...
  const distributorBalance = await alphaToken.balanceOf(rewardsDistributor.address);
  if (distributorBalance.lt(rewardTotal)) {
    console.log("Funding RewardsDistributor...");
    const fundTx = await alphaToken.transfer(rewardsDistributor.address, rewardTotal);
    await fundTx.wait();
  }
  ...
  if (!hasSetSpeeds) {
    ...
    const tx = await acm.giveCallPermission(rewardsDistributor.address, setSpeedsSig, deployer);
    await tx.wait();
  }
  ...
  if (!hasSetLastBlocks) {
    ...
    const tx = await acm.giveCallPermission(rewardsDistributor.address, setLastBlocksSig, deployer);
    await tx.wait();
  }
